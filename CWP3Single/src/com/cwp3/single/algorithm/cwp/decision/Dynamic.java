...
//            dpCraneSelectBay.getDpPair().setCranePosition(cwpBay.getWorkPosition());
        ...
        for (int i = 1; i < craneNum; i++) {
            ...
            if (dpCraneSelectBay != null) {
//                dpCraneSelectBay.getDpPair().setCranePosition(cwpBay.getWorkPosition()); //position of the crane???
                if (dpCraneSelectBay.getDpWorkTime() > 0) {
...
//                    cwpCrane1.setDpCurrentWorkPosition(dpPair1.getCranePosition());
                }
            }
        }
    ...
    private boolean better(DPResult cur_dp, DPResult dpResult) {
        if (cur_dp.getDpFeatureCode() > dpResult.getDpFeatureCode()) {
            ...
            if (cur_dp.getDpFeatureCode() == dpResult.getDpFeatureCode()) {
//                return cur_dp.getDpWorkTime() > dpResult.getDpWorkTime() || (cur_dp.getDpWorkTime().longValue() == dpResult.getDpWorkTime() && cur_dp.getDpDistance() < dpResult.getDpDistance());
